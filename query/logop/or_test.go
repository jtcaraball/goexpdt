...
func TestOr_Encoding_Nil(t *testing.T) {
	tree, _ := test.NewMockTree(1, nil)
	ctx := query.BasicQContext(tree)

	vcmp := logop.Or{test.Trivial(true), test.Trivial(true)}
	icmp1 := logop.Or{nil, test.Trivial(true)}
	icmp2 := logop.Or{test.Trivial(true), nil}

	ce := "Or: Invalid encoding with nil ctx"
	che1 := "Or: Invalid encoding of nil child (1)"
	che2 := "Or: Invalid encoding of nil child (2)"
