...
func runFullVar(
	...
	filePath := sfdtest.CNFName(compSufix(leafValue), id, simplify)
	if simplify {
...
func compSufix(val bool) string {
...
func TestVar_Encoding_AllPos(t *testing.T) {
	sfdtest.AddCleanup(t, compSufix(true), false)
	tree := genTree()
	for i, tc := range allPosTests {
		t.Run(tc.name, func(t *testing.T) {
			runFullVar(t, i, tc.expCode, tc.val, tree, true, false)
		})
	}
}

func TestVar_Encoding_AllNeg(t *testing.T) {
	sfdtest.AddCleanup(t, compSufix(false), false)
	tree := genTree()
	for i, tc := range allNegTests {
		t.Run(tc.name, func(t *testing.T) {
			runFullVar(t, i, tc.expCode, tc.val, tree, false, false)
		})
	}
}
...
func TestVar_Simplified_AllPos(t *testing.T) {
	...
	for i, tc := range allPosTests {
		t.Run(tc.name, func(t *testing.T) {
			runFullVar(t, i, tc.expCode, tc.val, tree, true, true)
		})
	}
}
...
func TestVar_Simplified_AllNeg(t *testing.T) {
	...
	for i, tc := range allNegTests {
		t.Run(tc.name, func(t *testing.T) {
			runFullVar(t, i, tc.expCode, tc.val, tree, false, true)
		})
	}
}
